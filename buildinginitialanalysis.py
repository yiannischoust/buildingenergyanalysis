...
try: # Normal offline run
    ...
    data = pd.read_excel(r'C:\Users\Yiannis\python\pv\BD4NRG_Pilot3_ASM_Headquarters_Power_Profiles.xlsx',sheet_name="Building")
    buildingdata = data.values.tolist()
    data = pd.read_excel(r'C:\Users\Yiannis\python\pv\BD4NRG_Pilot3_ASM_Headquarters_Power_Profiles.xlsx',sheet_name="Photovoltaic")
    pvdata = data.values.tolist()
except: # For Colab 
    wdata = pd.read_csv (r'/content/drive/MyDrive/Colab Notebooks/BD4NRG.csv', delimiter=';')
    weatherdata = wdata.values.tolist()
    data = pd.read_excel(r'/content/drive/MyDrive/Colab Notebooks/BD4NRG_Pilot3_ASM_Headquarters_Power_Profiles.xlsx',sheet_name="Building")
    buildingdata = data.values.tolist()
    data = pd.read_excel(r'/content/drive/MyDrive/Colab Notebooks/BD4NRG_Pilot3_ASM_Headquarters_Power_Profiles.xlsx',sheet_name="Photovoltaic")
    pvdata = data.values.tolist()
...
def wdatetotimestamp(weatdata):# Convert date columns into timestamp from weather data and fix NaN values

    wtimestring = ""
    for i in range(len(weatdata)):
        wtimestring = str(int(weatdata[i][0])) + "+01:00"
        weatdata[i][0] = datetime.datetime.strptime(wtimestring,"%Y%m%d%H%M%z").timetuple()
...
def pvcleanup(pv,pvtime): # Clean up power data
    for i in range(len(pv)):
        if pv[i][4] < 0:
            pv[i][4] = -pv[i][4] # Retrieve absolute power values to correct reverse power flow values
        if pv[i][4] < 100 and ( pvtime[i].tm_hour > 17 or pvtime[i].tm_hour < 7 ):
            pv[i][4] = 0 # Eliminates noise - Power produced after 5pm and before 7am less than 100 Watt is set to zero
    return pv

def add10mintohourly(buildingdata, btime): # Convert 10-minute values to hourly Wh values
    bhourly = [] #List Containing the timestamp and the energy consumed
    hourcounter = 0
    hourlyproduction = 0
    currenthour = 0
    for i in range(len(btime)-1): # Omit the last element of the list to solve the out of index on btime[i+1] error
        hourlyproduction += buildingdata[i][4]
        hourcounter += 1 # Some elements are missing, not always 6 per hour, so they are counted to calculate Wh
        if btime[i].tm_hour != btime[i+1].tm_hour:
            ...
            bhourly.append([time.struct_time(tuple(currenthour)),hourlyproduction/hourcounter])
            hourcounter = 0
            hourlyproduction = 0
...
def keepmatchingdatesonly(bhour,pvhour,wdata): # Keep only the data for each set corresponding to the same dates and times
    ...
    lateststart = 0
    earliestfinish = 0
    lateststart = max(time.mktime(bhour[1][0]),time.mktime(pvhour[1][0]),time.mktime(wdata[1][0]))
    earliestfinish = min(time.mktime(bhour[-1][0]),time.mktime(pvhour[-1][0]),time.mktime(wdata[-1][0]))
...
#def duplicatetimeremover(bhour):

def fixtimedata(bhour):
    bhournew = []
    fillervalue = []
    meanval = []
    for i in range(1,len(bhour)-1):
        if (datetime.datetime.fromtimestamp(time.mktime(bhour[i+1][0])) - datetime.datetime.fromtimestamp(time.mktime(bhour[i][0])) != datetime.timedelta(hours=1)):
            if bhour[i][0] == bhour[i-1][0] and ((datetime.datetime.fromtimestamp(time.mktime(bhour[i][0])) - datetime.datetime.fromtimestamp(time.mktime(bhour[i+1][0]))) == datetime.timedelta(hours=2)):
               # If the time is the same as the previous cell and two hours later in the one after that then it is just DST so add the value normally

                bhournew.append([(datetime.datetime.fromtimestamp(time.mktime(bhour[i][0])) + datetime.timedelta(hours=1)).timetuple(),bhour[i][1]])
            if bhour[i][0] != bhour[i-1][0] and ((datetime.datetime.fromtimestamp(time.mktime(bhour[i+1][0])) - datetime.datetime.fromtimestamp(time.mktime(bhour[i][0])))  == datetime.timedelta(hours=2)):
                #print(bhour[i][0])
                fillervalue = [(datetime.datetime.fromtimestamp(time.mktime(bhour[i][0])) + datetime.timedelta(hours=1)).timetuple(),bhour[i-24][1]]
                ''' For using weekly averages to replace the value - not working
                for j in range(-7,7):
                    meanval.append(bhour[i + j*24][1])
                print(meanval)
                meanval = stat.mean(meanval)
                fillervalue = [(datetime.datetime.fromtimestamp(time.mktime(bhour[i][0])) + datetime.timedelta(hours=1)).timetuple(),meanval]
                print(fillervalue)
                '''
                bhournew.append(bhour[i])
                bhournew.append(fillervalue) # Fill in the value of the previous day for the missing data
...
# Data for PV
pvtime = datetotimestamp(pvdata)
pvcleanup(pvdata,pvtime)
pvhourly = add10mintohourly(pvdata, pvtime)

# Keep only the common dates among all data sets
bhourly, pvhourly, weatherdata = keepmatchingdatesonly(bhourly, pvhourly, weatherdata)
...
for i in range(len(bhourly)): # Check if dates match and prepare the lists for each value
    if bhourly[i][0] == pvhourly[i][0] == weatherdata[i][0]:
        try: # Will raise an error when trying the time when DST starts, ignore that value -will omit a (erroneous) value of the original index
            ...
            pass
            '''
            wvalues2humid.append(weatherdata[i-1][2])
            wvalues3wind.append(weatherdata[i-1][3])
            wvalues4cloud.append(weatherdata[i-1][4])
            wvalues5rad.append(weatherdata[i-1][5])
            '''
    else:
